                    ...
                    <tr class="hover:bg-gray-50/50 transition-colors">
                        ...
                        <td class="px-8 py-6">
                            ...
                            @elseif($reservation->status === 'expiré')
                                <span class="px-3 py-1 text-[10px] font-bold uppercase rounded-full bg-gray-100 text-gray-600 tracking-wider">Expiré</span>
                            @else
                        ...
                        <td class="px-8 py-6 flex items-center gap-3">
                            <a href="{{ route('visits.show', $reservation->visit) }}" class="text-gray-400 hover:text-emerald-600 transition-colors" title="Voir la visite">
                            ...
                            @if(in_array($reservation->status, ['annulé', 'expiré']))
                                <form action="{{ route('voyageur.reservations.destroy', $reservation) }}" method="POST" class="inline" onsubmit="return confirm('Voulez-vous vraiment retirer cette réservation de votre historique ?');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="text-gray-400 hover:text-red-600 transition-colors" title="Supprimer de l\'historique">
                                        <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
