<!DOCTYPE html>
<html lang="fr">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Créer une visite — Atlas Trek</title>
    <style>
        body {
            font-family: Arial, sans-serif;
            max-width: 650px;
            margin: 40px auto;
            padding: 0 20px;
        }

        h1 {
            color: #2d6a4f;
        }

        .form-group {
            margin-bottom: 18px;
        }

        label {
            display: block;
            font-weight: bold;
            margin-bottom: 6px;
            color: #333;
        }

        input,
        textarea,
        select {
            width: 100%;
            padding: 10px;
            border: 1px solid #ccc;
            border-radius: 4px;
            font-size: 15px;
            box-sizing: border-box;
        }

        textarea {
            height: 120px;
            resize: vertical;
        }

        .btn {
            display: inline-block;
            padding: 10px 20px;
            background: #2d6a4f;
            color: white;
            text-decoration: none;
            border-radius: 4px;
            border: none;
            cursor: pointer;
            font-size: 15px;
        }

        .btn-secondary {
            background: #888;
        }

        .error {
            color: #c0392b;
            font-size: 13px;
            margin-top: 4px;
        }

        .alert-errors {
            background: #f8d7da;
            color: #721c24;
            padding: 12px;
            border-radius: 4px;
            margin-bottom: 20px;
        }

        small {
            color: #888;
            font-size: 12px;
        }
    </style>
</head>

<body>

    <h1>➕ Créer une nouvelle visite</h1>

    {{-- Affiche un résumé des erreurs de validation si elles existent --}}
    @if($errors->any())
    <div class="alert-errors">
        <strong>Corrigez les erreurs suivantes :</strong>
        <ul>
            @foreach($errors->all() as $error)
            <li>{{ $error }}</li>
            @endforeach
        </ul>
    </div>
    @endif

    {{--
        Le formulaire envoie les données en POST vers la route visits.store.
        @csrf est obligatoire : Laravel vérifie ce token pour éviter les attaques CSRF.
    --}}
    <form action="{{ route('visits.store') }}" method="POST" enctype="multipart/form-data">
        @csrf

        <!-- Titre -->
        <div class="form-group">
            <label for="title">Titre de la visite *</label>
            {{-- old('title') remet la valeur saisie si le formulaire a été resoumis après erreur --}}
            <input type="text" id="title" name="title" value="{{ old('title') }}"
                placeholder="Ex: Trek dans le Haut Atlas">
            @error('title')
            <div class="error">{{ $message }}</div>
            @enderror
        </div>

        <!-- Description -->
        <div class="form-group">
            <label for="description">Description *</label>
            <textarea id="description" name="description"
                placeholder="Décrivez la visite en détail...">{{ old('description') }}</textarea>
            @error('description')
            <div class="error">{{ $message }}</div>
            @enderror
        </div>

        <!-- Lieu -->
        <div class="form-group">
            <label for="location">Lieu *</label>
            <input type="text" id="location" name="location" value="{{ old('location') }}"
                placeholder="Ex: Toubkal, Marrakech">
            @error('location')
            <div class="error">{{ $message }}</div>
            @enderror
        </div>

        <!-- Prix -->
        <div class="form-group">
            <label for="price">Prix (MAD) *</label>
            <input type="number" id="price" name="price" value="{{ old('price') }}"
                step="0.01" min="0" placeholder="Ex: 350.00">
            @error('price')
            <div class="error">{{ $message }}</div>
            @enderror
        </div>

        <!-- Durée -->
        <div class="form-group">
            <label for="duration">Durée (en heures) *</label>
            <input type="number" id="duration" name="duration" value="{{ old('duration') }}"
                min="1" placeholder="Ex: 8">
            <small>Entrez le nombre d'heures (ex: 24 pour 1 journée complète)</small>
            @error('duration')
            <div class="error">{{ $message }}</div>
            @enderror
        </div>

        <!-- Difficulté -->
        <div class="form-group">
            <label for="difficulty">Niveau de difficulté *</label>
            <select id="difficulty" name="difficulty">
                <option value="">-- Choisissez un niveau --</option>
                {{-- old('difficulty') re-sélectionne l'option choisie en cas d'erreur --}}
                <option value="facile" {{ old('difficulty') == 'facile'    ? 'selected' : '' }}>🟢 Facile</option>
                <option value="moyen" {{ old('difficulty') == 'moyen'     ? 'selected' : '' }}>🟡 Moyen</option>
                <option value="difficile" {{ old('difficulty') == 'difficile' ? 'selected' : '' }}>🔴 Difficile</option>
            </select>
            @error('difficulty')
            <div class="error">{{ $message }}</div>
            @enderror
        </div>
        <div>
            <label>Image :</label>
            <input type="file" name="image">
        </div>

        <!-- Boutons -->
        <div style="display: flex; gap: 10px;">
            <button type="submit" class="btn">💾 Enregistrer la visite</button>
            <a href="{{ route('visits.index') }}" class="btn btn-secondary">Annuler</a>
        </div>

    </form>

</body>

</html>